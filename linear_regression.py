...
class LinearRegression:
    ...
    def fit(self, x: np.ndarray, y: np.ndarray) -> LinearRegression:
        ...
        w0 = self.descent.step(x,y)
        is_nan = np.isnan(w0).sum()
        tolerance_level = (np.linalg.norm(w0, ord = 2))**2

        self.max_iter -= 1
        while (self.max_iter > 0 and  is_nan == 0 and self.tolerance < tolerance_level):
            self.loss_history += [self.descent.calc_loss(x,y)]
