...
class BaseDescent:
    ...
    def __init__(self, dimension: int, lambda_: float = 1e-3, loss_function: LossFunction = LossFunction.MSE):
        ...
        #self.w: np.ndarray  = np.array([0.45,  0.27, 0.76])  ### !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        L = y - self.predict(x)
        return np.dot(np.transpose(L),L) * 1/y.size
...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        gr = (np.dot(np.transpose(x), self.predict(x)) - np.dot(np.transpose(x),y)) * 2/y.size
        return(gr)
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        ind_rnd = np.random.randint(0, y.size, self.batch_size)
        x_rnd = x[ind_rnd]
        y_rnd = y[ind_rnd]
        return super().calc_gradient(x_rnd, y_rnd)
